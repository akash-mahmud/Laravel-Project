      ...
      <table class="table table-borderless table-hover mt-5">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <form class="text-right" action="{{route('user.sales.invoices.delete_item',['id' => $user->id ,'invoice_id' => $invoice->id , 'item_id' => $item->id ])}}" method="POST">
        ...
        <tr>
          ...
          <td class="text-right">{{$totalPayable = $invoice->items()->sum('total')}}</td>
        </tr>
        <tr>
          <td colspan="2"><Button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#newReceiptForInvoice"><i class="fa fa-plus"></i> Add Receipt</Button></td>
          <td colspan="2" class="text-right"><strong>Paid :</strong> </td>
          <td class="text-right">{{$totalPaid = $invoice->receipts()->sum('amount')}}</td>
        </tr>
        <tr>
...
<div class="modal fade" id="newReceiptForInvoice" tabindex="-1" role="dialog" aria-labelledby="newReceiptForInvoiceModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    {!! Form::open(['route' => ['user.receipt.store', [$user->id , $invoice ->id]],'method'=> 'post']) !!}
    <div class="modal-content">
      <div class="modal-header">
